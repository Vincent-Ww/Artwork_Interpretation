...
class BeamSearch(object):
    ...
    def apply(self, visual: utils.TensorOrSequence, out_size=1, return_probs=False, is_sample=False, **kwargs):
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                visual, outputs = self.iter(t, visual, outputs, return_probs, is_sample, **kwargs)
...
#         print("top-k sampling")
        ...
        next_token_logscores = transformers.top_k_top_p_filtering(candidate_logprob_flatten, top_k=10)
        probs = F.softmax(next_token_logscores, dim=-1)
        next_token = torch.multinomial(probs, num_samples=1).squeeze(1)
        return next_token.unsqueeze(dim=0), probs[0][next_token].unsqueeze(dim=0)

    def iter(self, t: int, visual: utils.TensorOrSequence, outputs, return_probs, is_sample, **kwargs):
