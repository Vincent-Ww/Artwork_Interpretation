...
from data.dataset import AP_Dataset0, AP_Dataset, APeval_Dataset, SA_Dataset, SAeval_Dataset
import evaluation
...
loss_file = "output_logs/loss_file_sa.txt"
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(train_data_loader)) as pbar:
        for it, batch in enumerate(train_data_loader):
            ...
            if it % 200 == 0:
                ...
                train_loss = evaluate_loss(model, train_data_loader, loss_fn, text_field)
                print(f"the {it}th iteration: Val loss {val_loss}  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!2")
                with open(loss_file, 'a+') as f:
                    f.write(f'the {it}th iteration, Train loss {train_loss} \n')                 
...
#         text_field.build_vocab(train_dataset, val_dataset, min_freq=5)
    ...
    #     artpedia_train = AP_Dataset0(ap_train_dataset, train_myidx, text_field, max_detections=50, lower=True, remove_punctuation=True,
    #                        tokenize='spacy')
    #     artpedia_val = AP_Dataset0(ap_val_dataset, val_myidx, text_field, max_detections=50, lower=True, remove_punctuation=True,
    #                        tokenize='spacy')
    #     artpedia_test = AP_Dataset0(ap_test_dataset, test_myidx, text_field, max_detections=50, lower=True, remove_punctuation=True,
    #                        tokenize='spacy')
    
    artpedia_train = AP_Dataset(ap_train_dataset, train_item2imgCapIdx, text_field, max_detections=50, lower=True, remove_punctuation=True,
                       tokenize='spacy')
    artpedia_val = AP_Dataset(ap_val_dataset, val_item2imgCapIdx, text_field, max_detections=50, lower=True, remove_punctuation=True,
                       tokenize='spacy')
    artpedia_test = AP_Dataset(ap_test_dataset, test_item2imgCapIdx, text_field, max_detections=50, lower=True, remove_punctuation=True,
                       tokenize='spacy')
...
#     dict_dataset_test = test_dataset.image_dictionary({'image': image_field, 'text': RawField()})
    ...
    dict_artpedia_train = APeval_Dataset(ap_train_dataset, train_myidx, text_field, max_detections=50, lower=True, remove_punctuation=True, tokenize='spacy')
    dict_artpedia_val = APeval_Dataset(ap_val_dataset, val_myidx, text_field, max_detections=50, lower=True, remove_punctuation=True, tokenize='spacy')
...
#         # artpedia dataloader
#         train_data_loader = TorchDataLoader(artpedia_train, batch_size=args.batch_size, 
