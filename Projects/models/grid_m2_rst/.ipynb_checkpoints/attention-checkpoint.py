...
# *****************
# m: dimension of meshed memory vector
class ScaledDotProductAttentionMemory(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        att = torch.matmul(q, k) / np.sqrt(self.d_k)  # (b_s, h, nq, nk+m)
        if attention_weights is not None:
...
# **********************
# RSTNet: add grid-augumented module
class ScaledDotProductGeometryAttention(nn.Module):
    '''
    Scaled dot-product attention
    '''

    def __init__(self, d_model, d_k, d_v, h, m, dropout=.1, comment=None):
        '''
        :param d_model: Output dimensionality of the model
        :param d_k: Dimensionality of queries and keys
        :param d_v: Dimensionality of values
    ...
    def forward(self, queries, keys, values, box_relation_embed_matrix, attention_mask=None, attention_weights=None):
        ...
        # 只有前nk是attention，后m是memory vector
        if attention_weights is not None:
            att = torch.cat([att[:, :, :, :nk] * attention_weights, att[:, :, :, nk:]], -1)
        if attention_mask is not None:
            att[:, :, :, :nk] = att[:, :, :, :nk].masked_fill(attention_mask, -np.inf)
        ...
        # grid-augumented module
        w_g = box_relation_embed_matrix          # (b_s, h, nq, nk)
        w_a = att                                # (b_s, h, nq, nk+m)
        # w_mn = torch.cat([torch.torch.log(torch.clamp(w_g, min=1e-6)) + w_a[:, :, :, :nk], w_a[:, :, :, nk:]], -1)
        w_a[:, :, :, :nk].add_(torch.log(torch.clamp(w_g, min=1e-6)))    # in-place operation
        # print(torch.all(w_mn0 == w_a))
        # print(w_mn0)
        # print(w_a)
        # w_mn = torch.log(torch.clamp(w_g, min=1e-6)) + w_a             # grid-augumented module

        # w_mn = torch.softmax(w_mn, -1)  # bs * 8 * r * r
        w_mn = torch.softmax(w_a, -1)

        # att = self.dropout(w_mn)    # does not exist in m2
        att = w_mn
...
class MultiHeadGeometryAttention(Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, identity_map_reordering=False, can_be_stateful=False,
        ...
        self.attention = ScaledDotProductGeometryAttention(d_model=d_model, d_k=d_k, d_v=d_v, h=h, m=40, comment=comment)
