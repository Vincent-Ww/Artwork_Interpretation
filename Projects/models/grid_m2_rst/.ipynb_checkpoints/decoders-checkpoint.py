...
from models.grid_m2_rst.language_model import LanguageModel
from models.grid_m2_rst.attention import MultiHeadAttention, MultiHeadAdaptiveAttention
from models.grid_m2_rst.utils import sinusoid_encoding_table, PositionWiseFeedForward
...
class MeshedDecoderLayer(Module):
    ...
    # input: caption
    # 先对输入的caption来一个self-attention layer。然后进入论文中描述的meshed cross-attention layer
    def forward(self, input, enc_output, mask_pad, mask_self_att, mask_enc_att, pos=None):
        self_att = self.self_att(input, input, input, mask_self_att)
        self_att = self.lnorm1(input + self.dropout1(self_att))
        self_att = self_att * mask_pad

        # meshed-memory attention   此代码只针对3层encoder
        enc_att1 = self.enc_att(self_att, enc_output[:, 0] + pos, enc_output[:, 0], mask_enc_att) * mask_pad
        enc_att2 = self.enc_att(self_att, enc_output[:, 1] + pos, enc_output[:, 1], mask_enc_att) * mask_pad
        enc_att3 = self.enc_att(self_att, enc_output[:, 2] + pos, enc_output[:, 2], mask_enc_att) * mask_pad
...
class MeshedDecoderAdaptiveLayer(Module):
    def __init__(self, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1, self_att_module=None,
                 enc_att_module=None, self_att_module_kwargs=None, enc_att_module_kwargs=None):
        super(MeshedDecoderAdaptiveLayer, self).__init__()
        self.self_att = MultiHeadAttention(d_model, d_k, d_v, h, dropout, can_be_stateful=True,
                                           attention_module=self_att_module,
                                           attention_module_kwargs=self_att_module_kwargs)
    ...
    def forward(self, input, enc_output, mask_pad, mask_self_att, mask_enc_att, language_feature=None, pos=None):
        ...
        # meshed-memory attention   此代码只针对3层encoder
        enc_att1 = self.enc_att(self_att, enc_output[:, 0] + pos, enc_output[:, 0], mask_enc_att, language_feature=language_feature) * mask_pad
        enc_att2 = self.enc_att(self_att, enc_output[:, 1] + pos, enc_output[:, 1], mask_enc_att, language_feature=language_feature) * mask_pad
        enc_att3 = self.enc_att(self_att, enc_output[:, 2] + pos, enc_output[:, 2], mask_enc_att, language_feature=language_feature) * mask_pad
...
class MeshedDecoder(Module):
    def __init__(self, vocab_size, max_len, N_dec, padding_idx, language_model_path='models/grid_m2_rst/language_context.pth',
        ...
        self.pos_emb = nn.Embedding.from_pretrained(sinusoid_encoding_table(max_len + 1, d_model, 0), freeze=True)   # 固定positional embedding
        ...
        self.layers = ModuleList(
            [MeshedDecoderLayer(d_model, d_k, d_v, h, d_ff, dropout, self_att_module=self_att_module,
                 ...
                 if i < N_dec \
                 else MeshedDecoderAdaptiveLayer(d_model, d_k, d_v, h, d_ff, dropout, self_att_module=self_att_module,
                                           enc_att_module=enc_att_module, self_att_module_kwargs=self_att_module_kwargs,
    ...
    def forward(self, input, encoder_output, mask_encoder, pos):
        ...
        # rstnet：adaptive attention specific
