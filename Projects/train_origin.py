...
loss_file = "output_logs/loss_file_gridm2.txt"
...
def evaluate_metrics(model, dataloader, text_field):
    ...
    scores, _ = evaluation.compute_scores(gts, gen, spice=True)
    return scores


def train_xe(model, train_data_loader, val_data_loader, optim, text_field, e):
    # Training with cross-entropy
    model.train()
    scheduler.step()
    running_loss = .0
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(train_data_loader)) as pbar:
        for it, (detections, captions) in enumerate(train_data_loader):
            detections, captions = detections.to(device), captions.to(device)
            out = model(detections, captions)
            optim.zero_grad()
            ...
            if it % 2500 == 0 and it != 0:
                ...
                torch.save({
                    ...
                    'best_cider': None,
                    'use_rl': None,
                }, 'saved_models/{}_last_{}epoch_{}it.pth'.format(args.exp_name, e, it))
...
#                                      attention_module_kwargs={'m': args.m})
    ...
    print("building dataloader: done !!")
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        if not use_rl:
            print("~~ training by xe ~~")
            train_loss = train_xe(model, dataloader_train, dataloader_val, optim, text_field, e)
            with open(loss_file, 'a+') as f:
                f.write(f'Epoch {e}, Training loss {train_loss} \n')
        ...
        if exit_train:
            writer.close()
            print("exit_train, forced to stop!!!!!!!!!!!!!!!")
            break
