...
#                     return_probs=False, is_sample=False, **kwargs):
# #         if is_sample:
# #             print('generation by top-k sampling')
# #         elif not is_sample:
# #             print(f"generation by beam search with beam size {beam_size}")
#         bs = BeamSearch(self, max_len, eos_idx, beam_size)
#         return bs.apply(visual, out_size, return_probs, is_sample, **kwargs)
    
        # visual: ROI features
    def beam_search(self, visual: utils.TensorOrSequence, max_len: int, eos_idx: int, beam_size: int, out_size=1,
                    return_probs=False, is_sample=False, top_k=10, top_p=0.8, **kwargs):
        bs = BeamSearch(self, max_len, eos_idx, beam_size)
        return bs.apply(visual, out_size, return_probs, is_sample, top_k=top_k, top_p=top_p, **kwargs)
